...
void GLViewer::setupBasicScene()
{
	...
	try {
		...
		m_Scene.loadLightsIntoPrograms();
		...
		TriangleNetObject sphere(&sphereMatRef, &geomSphere, &prog);
		sphere.scale(glm::dvec3(0.5));

		m_Scene.addObject(sphere);
	} catch (const std::exception &ex) {
		std::cerr << "Error creating basic scene: " << ex.what() << std::endl;
	}
